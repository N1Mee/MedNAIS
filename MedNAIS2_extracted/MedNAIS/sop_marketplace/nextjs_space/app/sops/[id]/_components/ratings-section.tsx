...
export function RatingsSection({ sopId }: RatingsSectionProps) {
  ...
  const checkRatingAccess = async () => {
    ...
    try {
      // Fetch SOP details to check ownership and price
      ...
      // Authors cannot rate their own SOPs
      if (sopData.authorId === session.user.id) {
        setCanRate(false);
        setCheckingAccess(false);
        return;
      }

      // If SOP is free, anyone can rate
      if (sopData.price === 0) {
        setCanRate(true);
        setCheckingAccess(false);
        return;
      ...
      // For paid SOPs, check if user has purchased
      const purchaseResponse = await fetch(`/api/sops/${sopId}/purchase-status`);
      ...
      console.error("Error checking rating access:", error);
      setCanRate(false);
    } finally {
      setCheckingAccess(false);
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        {session?.user && (
          <button
            onClick={() => canRate && setShowRatingForm(!showRatingForm)}
            disabled={!canRate || checkingAccess}
            className={`px-4 py-2 rounded-lg transition ${
              canRate && !checkingAccess
                ? "bg-[#E63946] text-white hover:bg-[#E63946]/90 cursor-pointer"
                : "bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 cursor-not-allowed"
            }`}
            title={!canRate ? "Purchase this SOP to leave a rating" : "Write a Review"}
          >
            {checkingAccess ? "Loading..." : showRatingForm ? "Cancel" : "Write a Review"}
          </button>
        )}
      </div>
