...
export async function POST(request: NextRequest) {
  try {
    ...
    // Include session_id in success URL for payment verification
    const isCartCheckout = sopIds && Array.isArray(sopIds);
    const successUrl = isCartCheckout
      ? `${origin}/cart?payment=success&session_id={CHECKOUT_SESSION_ID}`
      : `${origin}/sops/${sopId}?payment=success&session_id={CHECKOUT_SESSION_ID}`;
